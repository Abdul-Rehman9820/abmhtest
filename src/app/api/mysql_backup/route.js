    ...
    async function executefetchQuery(connection, usersearch, stype) {
      ...
      if(stype == "search"){
        ...
        return new Promise((resolve, reject) => {
          ...
          const query = 'SELECT abmh_doctors.*, abmh_specialty.specialty_slug,abmh_specialty.specialty_name FROM abmh_doctors LEFT JOIN abmh_specialty ON abmh_doctors.SpecialtyID = abmh_specialty.id WHERE ( abmh_doctors.isactive = 1 AND (abmh_doctors.DrFullName LIKE ? OR abmh_doctors.DoctorLaymanSearch LIKE ? OR abmh_specialty.specialty_name LIKE ?) ) ORDER BY abmh_doctors.DoctorFName ASC';
