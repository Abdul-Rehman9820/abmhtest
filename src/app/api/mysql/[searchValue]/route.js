...
export async function GET(req, content) {
  ...
  try {
    ...
    const searchValue = content.params.searchValue; // Get the search value from the URL

    async function executefetchQuery(connection, searchValue) {
      ...
      return new Promise((resolve, reject) => {
        ...
        const wildcardedSearchValue = `%${searchValue}%`; 

    
        connection.query(query, [wildcardedSearchValue], (err, result) => {
          if (err) {
    ...
    const results = await executefetchQuery(connection, searchValue);
